...
impl FORateControlInput{
    pub fn new(bw_state:FOBandwidthUsage,
...
impl FovOptixStruct{
    ...
    pub fn new(trendline: FOTrendlineEstimator, aimd: FOAimdRateControl, bit: FOBitrateEstimator, rc: FORateControlInput, bitrate_init_mbps : f64 , ) -> Self{
        Self{
    ...
    pub fn report_fovoptix_stats_server(&mut self, client_stats: NetworkStatisticsPacket, tx_time_s_taitime: TaiTime<0>, rx_time_s_taitime: TaiTime<0>, ) {
        ...
        self.trendline.UpdateTrendline(recv_delta_ms, send_delta_ms, (tx_time_s as f64 *0.001) as i64, (rx_time_s as f64 *0.001)as i64, client_stats.bytes_in_frame as i64);
        ...
        self.bit.Update((rx_time_s as f64*0.001) as i64,client_stats.bytes_in_frame as usize ,false);
        ...
        let mut delta=self.aimd.current_bitrate_/72. / 8. + prev_target_bitrate / 72. / 8. - 2. * (client_stats.bytes_in_frame as f64);
        ...
        self.data_hist.push_back(dataPoint::new((tx_time_s - self.first_data_send_timestamp as f32) as f64, self.smooth_delta));
        if self.data_hist.len()>20{
            self.data_hist.pop_front();
        }
        
        if self.data_hist.len()==20{
            self.gradient = self.FitLine_Get_Gradient(&self.data_hist).unwrap_or(self.gradient);
        }
...
pub struct FOAimdRateControl{  // renaming with FO, GCC_NADA has same terminology
...
impl FOAimdRateControl{
    ...
    pub fn ChangeState(&mut self,input:& FORateControlInput,
        at_time:i64) {
    ...
    pub fn Update(&mut self,input:&FORateControlInput,
         at_time:i64)-> f64 {
    ...
    pub fn ChangeBitrate(&mut self,input:& FORateControlInput,
         at_time:i64) {
            ...
            if !self.bitrate_is_initialized_ &&
            input.bw_state != FOBandwidthUsage::kBwOverusing{
                return;
            }
