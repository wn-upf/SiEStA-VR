...
pub struct FOTrendlineEstimator {
...
impl FOTrendlineEstimator {
    pub fn new() -> Self {
        Self {
            ...
            overuse_counter_: 0,
            hypothesis_: FOBandwidthUsage::kBwNormal,
            hypothesis_predicted_: FOBandwidthUsage::kBwNormal,
            //NetworkStatePredictor* network_state_predictor_;
    ...
    pub fn Detect(&mut self, trend: f64, ts_delta: f64, now_ms: i64) {
        if self.num_of_deltas_ < 2 {
            self.hypothesis_ = FOBandwidthUsage::kBwNormal;
            return;
        }
        let modified_trend =
        ...
        if modified_trend > self.threshold_ {
            ...
            if (self.time_over_using_ > self.overusing_time_threshold_ && self.overuse_counter_ > 1)
            {
                if trend >= self.prev_trend_ {
                    ...
                    self.hypothesis_ = FOBandwidthUsage::kBwOverusing;
                }
            }
        } else if modified_trend < -self.threshold_ {
            self.time_over_using_ = -1.0;
            self.overuse_counter_ = 0;
            self.hypothesis_ = FOBandwidthUsage::kBwUnderusing;
        } else {
            self.time_over_using_ = -1.0;
            self.overuse_counter_ = 0;
            self.hypothesis_ = FOBandwidthUsage::kBwNormal;
...
#[derive(PartialEq)]
pub enum FORateControlState {
    kRcHold = 0,
    kRcIncrease = 1,
    kRcDecrease = 2,
}
pub struct FORateControlInput {
    pub bw_state: FOBandwidthUsage,
    pub estimated_throughput: Option<f64>,
}
impl FORateControlInput {
    pub fn new(bw_state: FOBandwidthUsage, estimated_throughput: Option<f64>) -> Self {
        Self {
            bw_state: bw_state,
...
pub struct FovOptixStruct {
    // pub inter: FOInterArrival,  // defined, but UNUSED by original code
    pub trendline: FOTrendlineEstimator,
    pub aimd: FOAimdRateControl,
    pub bit: FOBitrateEstimator,
...
impl FovOptixStruct {
    pub fn new(
        trendline: FOTrendlineEstimator,
        aimd: FOAimdRateControl,
        bit: FOBitrateEstimator,
        rc: FORateControlInput,
        bitrate_init_mbps: f64,
    ) -> Self {
        Self {
            trendline,
            aimd,
            ...
            prev_target_bitrate_inrtc: bitrate_init_mbps * 1e6, // as bps
    ...
    pub fn FitLine_Get_Gradient(&self, packets: &VecDeque<dataPoint>) -> Option<f64> {
        if packets.len() > 2 {
            // Compute the "center of mass".
            let mut sum_x = 0.0;
            let mut sum_y = 0.0;
    ...
    pub fn report_fovoptix_stats_server(
        &mut self,
        client_stats: NetworkStatisticsPacket,
        tx_time_s: f32,
        rx_time_s: f32,
        now: TaiTime<0>,
    ) {
        ...
        self.trendline.UpdateTrendline(
            recv_delta_ms,
            send_delta_ms,
            (tx_time_s as f64 * 0.001) as i64,
            (rx_time_s as f64 * 0.001) as i64,
            client_stats.bytes_in_frame as i64,
        );

        threshold_c = self.trendline.current_threshold_for_testing.to_string();
        m_trend = self.trendline.current_trend_for_testing.to_string();

        if self.trendline.hypothesis_ == FOBandwidthUsage::kBwNormal {
            network_estimate = "normal".to_string(); // why use strings...?
            self.rc.bw_state = FOBandwidthUsage::kBwNormal;
        } else if self.trendline.hypothesis_ == FOBandwidthUsage::kBwOverusing {
            network_estimate = "overuse".to_string();
            self.rc.bw_state = FOBandwidthUsage::kBwOverusing;
        } else {
            network_estimate = "underuse".to_string();
            self.rc.bw_state = FOBandwidthUsage::kBwUnderusing;
        }

        if self.aimd.rate_control_state_ == FORateControlState::kRcHold {
            current_state = "hold".to_string();
        } else if self.aimd.rate_control_state_ == FORateControlState::kRcDecrease {
        ...
        self.bit.Update(
            (rx_time_s as f64 * 0.001) as i64,
            client_stats.bytes_in_frame as usize,
            false,
        );
        if self.bit.bitrate().is_some() {
            bitrate_estimate_rtc = self.bit.bitrate().unwrap();
            self.rc.estimated_throughput = Some(bitrate_estimate_rtc);
        }
        //else{
        ...
        //     self.rc.estimated_throughput=Some(b_s as f64);
        // }
        ...
        target_bitrate_bps_inrtc =
            (self.aimd.Update(&self.rc, time_millis as i64) / 1024.0 / 1024.0).to_string(); // using TaiTime reference
        ...
        let mut delta = self.aimd.current_bitrate_ / 72. / 8. + prev_target_bitrate / 72. / 8.
            - 2. * (client_stats.bytes_in_frame as f64);
        let normalize = delta / (self.aimd.current_bitrate_ / 72. / 8.);
        self.aimd.normalize_delta = normalize;
        self.accumulate_delta += delta;
        self.smooth_delta = 0.9 * self.smooth_delta + 0.1 * self.accumulate_delta;
        self.data_hist.push_back(dataPoint::new(
            (tx_time_s - self.first_data_send_timestamp as f32) as f64,
            self.smooth_delta,
        ));
        ...
        if self.data_hist.len() == 20 {
            self.gradient = self
                .FitLine_Get_Gradient(&self.data_hist)
        ...
        if self.aimd.rate_control_state_ == FORateControlState::kRcHold {
            ...
            next_state = "increase".to_string(); // unused, for logging in original github implementation.
        }
    }
}

pub struct FOAimdRateControl {
    // renaming with FO, GCC_NADA has same terminology
    ...
    pub rate_control_state_: FORateControlState,
...
impl FOAimdRateControl {
    pub fn new(send_side: bool) -> Self {
        Self {
            ...
            rate_control_state_: FORateControlState::kRcHold,
    ...
    pub fn ChangeState(&mut self, input: &FORateControlInput, at_time: i64) {
        match input.bw_state {
            FOBandwidthUsage::kBwNormal => {
                if self.rate_control_state_ == FORateControlState::kRcHold {
                    self.time_last_bitrate_change_ = at_time;
                    self.rate_control_state_ = FORateControlState::kRcIncrease;
                }
            }
            FOBandwidthUsage::kBwOverusing => {
                if self.rate_control_state_ != FORateControlState::kRcDecrease {
                    self.rate_control_state_ = FORateControlState::kRcDecrease;
                }
            }
            FOBandwidthUsage::kBwUnderusing => {
                self.rate_control_state_ = FORateControlState::kRcHold;
            }
            _ => {}
        }
    }
    pub fn GetNearMaxIncreaseRateBpsPerSecond(&mut self) -> f64 {
        ...
        let mut increase_rate_bps_per_second = avg_packet_size / response_time as f64;
    ...
    pub fn MultiplicativeRateIncrease(
        ...
        let mut multiplicative_increase = f64::max(current_bitrate * (alpha - 1.0), 1000.0);
    ...
    pub fn Update(&mut self, input: &FORateControlInput, at_time: i64) -> f64 {
    ...
    pub fn ChangeBitrate(&mut self, input: &FORateControlInput, at_time: i64) {
        ...
        if !self.bitrate_is_initialized_ && input.bw_state != FOBandwidthUsage::kBwOverusing {
            return;
        }
        ...
        match self.rate_control_state_ {
            FORateControlState::kRcHold => {}
            FORateControlState::kRcIncrease => {
            ...
            FORateControlState::kRcDecrease => {
...
impl FOBitrateEstimator {
    ...
    pub fn Update(&mut self, at_time: i64, amount: usize, in_alr: bool) {
        ...
        // symmetry.
        let sample_uncertainty = scale * (self.bitrate_estimate_kbps_ - bitrate_sample_kbps).abs()
