...
impl HevcParser {
    ...
    pub fn get_nal_units(&mut self) -> Vec<NalUnit> {
        ...
        while let Some(nal) = self.next_nal_unit() {
            println!(
                "Found NAL unit type {}, keyframe: {}",
                nal.nal_type, nal.is_keyframe
            );
            nal_units.push(nal);
        }
    ...
    pub fn get_frames(&mut self) -> Vec<Vec<u8>> {
        ...
        println!(
            "Processing {} NAL units for frame assembly",
            nal_units.len()
        );
        for (i, nal) in nal_units.iter().enumerate() {
            println!(
                "NAL #{}: type={}, keyframe={}, size={} bytes",
                i,
                nal.nal_type,
                nal.is_keyframe,
                nal.data.len()
        ...
        if !current_frame.is_empty() {
            frames.push(current_frame);
            println!(
                "Added final frame of size {} bytes",
...
impl ChunkedHevcEncoder {
    ...
    pub fn adjust_pacing(&mut self, frames_consumed_per_second: f32) {
        ...
        println!(
            "Pacing: consumption rate {:.1} fps, adjusted chunk delay to {:?}",
    ...
    fn check_buffer_status(&mut self) -> Result<()> {
        ...
        if self.current_child.is_none() {
            ...
            if buffer_size < self.min_frames_threshold {
                ...
                println!(
                    "Buffer low ({}/{} frames), starting new chunk",
                ...
                println!(
                    "Buffer below target ({}/{} frames), scheduling chunk in {:?}",
                ...
                println!(
                    "Buffer full ({}/{} frames), delaying next chunk",
                    buffer_size, self.target_frames_in_buffer
                );
    ...
    pub fn create_synthetic_frame(&mut self) -> bool {
        ...
        if let (Some(params), Some(keyframe)) = (&self.parameter_sets, &self.last_keyframe) {
            ...
            println!(
                "Created synthetic frame: {} bytes total (params: {}, keyframe: {})",
                frame.len(),
                params.len(),
                keyframe.len()
            );
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        std::thread::spawn(move || {
            ...
            println!(
                "Encoder stdout reader finished, read {}KB",
                total_bytes / 1024
            );
        });
    ...
    pub fn create_minimal_parameter_sets(&mut self) -> Vec<u8> {
        ...
        let vps: Vec<u8> = vec![
            ...
            0x03, 0x00, 0xb0, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x78, 0x02,
        ];

        // Create minimal SPS - this is simplified, might need customization
        let sps: Vec<u8> = vec![
            0x00, 0x00, 0x00, 0x01, 0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x80,
            0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x78, 0xa0, 0x03, 0xc0, 0x80, 0x10, 0xe5,
            0x96, 0x66, 0x69, 0x26, 0x64, 0x00, 0x20,
        ];

        // Create minimal PPS
        let pps: Vec<u8> = vec![0x00, 0x00, 0x00, 0x01, 0x44, 0x01, 0xc0, 0xf3, 0xc0, 0x02];
    ...
    pub fn process_incoming_packets(&mut self) -> Result<()> {
        ...
        if disconnected {
            ...
            println!(
                "Chunk complete: {} frames before, {} frames after",
            ...
            if frames_after == frames_before {
                ...
                println!(
                    "Warning: Chunk produced no frames ({} consecutive)",
                    self.consecutive_empty_chunks
                );

                if self.consecutive_empty_chunks >= 2 {
                    // Reduced threshold
                ...
                println!(
                    "Successfully added {} frames to buffer",
    ...
    pub fn start_chunk_encoding(&mut self) -> Result<()> {
        ...
        println!(
            "Starting chunk encoding at position {:.2}s with bitrate {}",
        ...
        while retry_count < max_retries {
            match self.create_encoding_process(start_position) {
                ...
                Err(e) => {
                    retry_count += 1;
                    eprintln!(
                        "Failed to start encoding process (attempt {}/{}): {}",
                    ...
                    if retry_count == 2 {
                        ...
                        println!(
                            "Toggling hardware acceleration to {}",
                    ...
                    if retry_count >= max_retries {
                        ...
                        if !self.frame_buffer.is_empty() {
                            ...
                            println!(
                                "Failed to start encoding, will try again at {:.2}s",
    ...
    pub fn attempt_recovery(&mut self) -> Result<()> {
        ...
        println!(
            "Attempting encoder recovery (attempt {})",
        ...
        match self.recovery_attempts % 4 {
            0 => {
                ...
                println!(
                    "Recovery: Restarting at current position {:.2}s",
                    self.next_chunk_position
                );
            }
            1 => {
                ...
                self.use_hardware_accel = !self.use_hardware_accel;
                println!(
                    "Recovery: Toggling hardware acceleration to {}",
                    self.use_hardware_accel
                );
            }
            2 => {
                ...
                println!(
                    "Recovery: Skipping ahead to {:.2}s",
                    self.next_chunk_position
                );
            }
            _ => {
    ...
    pub fn check_stall(
            ...
            && self.last_recovery_time.elapsed() > Duration::from_secs(3)
        {
            ...
            println!(
                "Stall detected (no frames for {}ms)",
...
impl HevcDecoder {
    pub fn new(framerate: u32, width: u32, height: u32) -> Result<Self> {
        ...
        std::thread::spawn({
            ...
            move || {
                println!(
                    "Starting decoder stdout reader thread, frame size: {}",
                    frame_size
                );
                ...
                println!(
                    "Decoder stdout reader finished: {} frames, {}KB total",
                    frames_read,
                    total_bytes / 1024
                );
            }
    ...
    pub fn process_packet(&mut self, packet: Vec<u8>) -> Result<()> {
            ...
            match self.packet_tx.try_send(packet.clone()) {
                Ok(_) => {
                    ...
                    if self.consecutive_errors > 0 {
                        println!(
                            "Decoder communication restored after {} errors",
        ...
        if self.consecutive_errors >= 10 {
            ...
            return Err(anyhow::anyhow!(
                "Decoder appears to be stalled after multiple errors"
            ));
        }

        // Just log warning for occasional errors
        eprintln!(
            "Warning: Decoder input channel full, packet dropped (error {}/10)",
            self.consecutive_errors
        );
...
fn main() -> Result<()> {
    ...
    while encoder.frames_available() == 0 && init_start.elapsed() < max_init_wait {
        ...
        if init_start.elapsed().as_secs() % 2 == 0 {
            println!(
                "Still waiting for frames... ({} seconds)",
    ...
    if encoder.frames_available() > 0 {
        println!(
            "Encoder initialized with {} frames",
            encoder.frames_available()
        );
    } else {
        ...
        if encoder.frames_available() == 0 {
            return Err(anyhow::anyhow!(
                "Failed to initialize encoder after multiple attempts"
            ));
        }

        println!(
            "Encoder finally initialized with {} frames",
            encoder.frames_available()
        );
    }
    ...
    println!(
        "Starting main loop with initial bitrate {}",
        INITIAL_BITRATE
    );
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        if fps_timer.elapsed() >= Duration::from_secs(2) {
            ...
            println!(
                "Time: {:.1}s | FPS: {} | Buffer: {} | Bitrate: {} | Position: {:.2}s",
                elapsed,
                frames_displayed / 2,
                buffer_size,
                current_bitrate,
