...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !output.status.success() {
        eprintln!("FFmpeg failed during frame extraction: {}", String::from_utf8_lossy(&output.stderr));
    ...
    if !concat_output.status.success() {
        eprintln!("FFmpeg failed during concatenation: {}", String::from_utf8_lossy(&concat_output.stderr));
