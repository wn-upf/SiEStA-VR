        ...
        let regular_frame = loop {
            match self.regular_encoder.next_frame().await {
                ...
                None => {
                    ...
                    self.regular_encoder
                        .start_chunking(self.encoder_bitrate_mbps)
                        .await;
        ...
        let max_frame = loop {
            match self.max_encoder.next_frame().await {
                ...
                None => {
                    ...
                    self.max_encoder
                        .start_chunking(self.maxencoder_bitrate_mbps)
                        .await;
                    tokio::time::sleep(Duration::from_millis(50)).await;
                }
            }
...
fn display_frame_pair_to_window(
    pair: &FramePair,
    server_ip: &IpAddr,
    display_frame_id: usize,
    window: &mut Window,
...
impl FrameSynchronizer {
    ...
    pub fn next_frame_pair(&mut self) -> Option<FramePair> {
        ...
        if let Some(pair) = self.get_exact_pair(self.next_display_id) {
            ...
            self.paired_history
                .push_back((self.next_display_id, self.next_display_id));
    ...
    fn try_fingerprint_sync(&mut self) -> Option<FramePair> {
        ...
        let regular_frames: Vec<(usize, &FramePair)> = self
            .pending_regular
            .iter()
        ...
        let reference_frames: Vec<(usize, &FramePair)> = self
            .pending_reference
            .iter()
        ...
        for &(reg_id, reg_pair) in &regular_frames {
            for &(ref_id, ref_pair) in &reference_frames {
                if let (Some(reg_raw), Some(ref_raw)) =
                    (&reg_pair.decoded_raw, &ref_pair.reference_raw)
                {
    ...
    fn check_sync_status(&mut self) {
        ...
        if now.duration_since(self.last_sync_time) > self.sync_interval {
            ...
            println!(
                ...
                self.sync_successes, self.sync_attempts, self.frames_dropped,
            );
    ...
    fn analyze_sync_history(&mut self) {
        ...
        if avg_drift.abs() > 0.5 {
            println!(
                "🔍 Detected consistent frame drift of {:.2} frames",
                avg_drift
            );
    ...
    fn try_cleanup_old_frames(&mut self) {
        ...
        let newly_dropped =
            (reg_before - self.pending_regular.len()) + (ref_before - self.pending_reference.len());
    ...
    rt.block_on(async {
        ...
        while running.load(Ordering::SeqCst) {
            ...
            match regular_rx.try_recv() {
                Ok((frame, frame_id)) => {
                    println!(
                        "Received regular frame #{} (size: {})",
                        frame_id,
                        frame.len()
                    );
                    let (raw, pixels, timestamp) =
                        decode_frame(&mut decoder, frame, frame_id, &client_ip);
            ...
            match max_rx.try_recv() {
                Ok((frame, frame_id)) => {
                    ...
                    let (raw, pixels, timestamp) =
                        decode_frame(&mut max_decoder, frame, frame_id, &client_ip);
            ...
            while let Some(complete_pair) = synchronizer.next_frame_pair() {
                ...
                if let Err(e) = pair_tx.send(complete_pair.clone()) {
                    ...
                    println!(
                        "Sent synchronized frame pair #{} to display",
                        complete_pair.frame_id
                    );
                    processed_frames = true;
                }
            }
...
fn decode_frame(
    ...
    print_prettyy!(
        DebugColor::Cyan,
        "[XRClient DECODE {}] Processing {} #{} (size: {} bytes)",
        ip,
        frame_display,
        frame_index,
        encoded_buffer.len(),
    ...
    if let Some((frame, inst)) = decoder.next_decoded_frame() {
        ...
        if let Some(pixels) = convert_rgb_to_u32(&frame, WIDTH_ENCODER, HEIGHT_ENCODER) {
            print_prettyy!(
                DebugColor::Green,
                "[XRClient DECODE {}] ✅ Successfully decoded frame #{}",
                ip,
                frame_index,
            );
            ...
            print_prettyy!(
                DebugColor::Red,
                "[XRClient DECODE {}] ERROR: Failed to convert decoded frame to RGB",
                ip,
            );
        ...
        if !decoder.priming_complete {
            ...
            print_prettyy!(
                DebugColor::Red,
                "[XRClient DECODE {}] No decoded frame available yet",
                ip,
            );
        }
        return (Vec::new(), Vec::new(), None);
...
fn display_frame_pair(pair: &FramePair, server_ip: &IpAddr, display_frame_id: usize) {
    ...
    let reference = match &pair.reference {
        ...
        None => {
            print_prettyy!(
                DebugColor::Red,
                "Reference frame is missing, cannot display",
            );
            return;
        }
    };
    ...
    print_prettyy!(
        DebugColor::Cyan,
        "Displaying frame pair #{} (decoded: {} pixels, reference: {} pixels)",
        display_frame_id,
        decoded.len(),
        reference.len(),
    );
    ...
    print_prettyy!(
        DebugColor::Green,
        "Displaying synced frame #{} (window size: {}x{})",
        display_frame_id,
        window_width,
        scaled_height,
    );
    ...
    DISPLAY_WINDOWS.with(|windows_cell| {
        ...
        if let Some(window) = windows.get_mut(server_ip) {
            ...
            if let Err(e) = window.update_with_buffer(&combined_buffer, window_width, scaled_height)
            {
                ...
                print_prettyy!(
                    DebugColor::Green,
                    "Successfully updated window with frame #{}",
                    display_frame_id,
                );
            }
        } else {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let regular_bitrate_mbps = 0.1;
    let max_bitrate_mbps = 140.0;
    let max_frames = 10E3 as usize;
    ...
    let encoder_thread = std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            let mut coordinator =
                EncodingCoordinator::new(input_path, regular_bitrate_mbps, max_bitrate_mbps);

            // Start initial chunking
            println!("Starting initial encoding chunks");
            coordinator
                .max_encoder
                .start_chunking(max_bitrate_mbps)
                .await;
            coordinator
                .regular_encoder
                .start_chunking(regular_bitrate_mbps)
                .await;
    ...
    println!(
        "Creating display window ({} x {})",
        window_width, scaled_height
    );
    ...
    while running_display.load(Ordering::SeqCst) && window.is_open() {
        ...
        match pair_rx.recv_timeout(Duration::from_millis(16)) {
            Ok(pair) => {
                ...
                if display_frame_pair_enhanced(
                    &pair,
                    &server_ip,
                    pair.frame_id,
                    &mut window,
        ...
        if last_sync_quality_check.elapsed() > Duration::from_secs(5) {
            if !sync_quality_history.is_empty() {
                ...
                if avg_quality > 0.2 {
                    // More than 20% difference is concerning
    ...
    println!(
        "Display loop completed - displayed {} frames",
        frames_displayed
    );
...
fn synchronize_frame_ids(
    regular_frames: &HashMap<usize, FramePair>,
    ...
    let mut common_ids: Vec<usize> = regular_frames
        .keys()
    ...
    if !common_ids.is_empty() {
        common_ids.sort();
        println!(
            "Synchronizing frame IDs: Adjusting next_display_id from {} to {}",
