...
fn scale_buffer(buffer: &[u32], width: usize, height: usize, new_width: usize, new_height: usize) -> Vec<u32> {
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        if is_playing {
            ...
            if window.is_key_pressed(Key::Left, minifb::KeyRepeat::Yes) {
                current_idx = if current_idx == 0 { frame_pairs.len() - 1 } else { current_idx - 1 };
                should_advance = true; // Force refresh
            }
        }
        ...
        match load_rgb_frame(lossy_path) {
            Ok(pixels) => {
                ...
                for y in 0..scaled_height {
                    for x in 0..scaled_width {
                        buffer[y * window_width + scaled_width + 10 + x] = scaled[y * scaled_width + x];
                    }
                }
            },
            Err(e) => {
        ...
        window.set_title(&format!(
            ...
            frame_number, current_idx + 1, frame_pairs.len(), status
        ));
