    ...
    for nxr in "${N_XR[@]}"; do
        for is_ul in "${IS_UL_BG[@]}"; do
            for bitrate in "${initial_bitrate_mbps[@]}"; do 
                ...
                name_folder=$(printf "sim_T%.0f_D%.0f_Br%.1f_PL%.03f_NXR%.0f_NBG%.0f_UL%.0f" \
                            "$simTime" "$distance" "$bitrate" "$PL" "$nxr" "$nbg" "$is_ul")
                ...
                if [ "$SERIAL_EXECUTION" -eq 0 ]; then  ## Parallel execution
                    ...
                    echo ./target/release/examples/XR_sim $simTime $mean_length $k_queue $rate_bps_src $rate_bps_queue $distance $bitrate $PL $nxr $nbg $is_ul >> "$temp_file"

                else                                    ## Serial execution
                    script -c "cargo run --release --example XR_sim $simTime $mean_length $k_queue $rate_bps_src $rate_bps_queue $distance $bitrate $PL $nxr $nbg $is_ul" "out_log.ans"
                    sleep 1
                    rm out_log.ans
                    rm -rf Video_Sink/*
