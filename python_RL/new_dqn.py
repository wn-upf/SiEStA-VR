...
DQN_SWEEP_CONFIG = {
    ...
    "parameters": {
        "algo": {"value": "DQN"},
        "learning_rate": {"values": [3e-4, 5e-5, 1e-4]},
        "buffer_size": {"values": [100_000, 500_000, 1_000_000]},
        "batch_size": {"values": [64, 128, 256]},
        "gamma": {"value": 0.99},
        "tau": {"value": 1.0}, # Standard for DQN (hard update)
        "train_freq": {"values": [4, 8, 16]}, # Train every N steps
        "gradient_steps": {"value": 1},
        "learning_starts": {"values": [10_000, 50_000]},
        "exploration_fraction": {"values": [0.1, 0.2]}, # Pct of training to anneal epsilon
        "exploration_final_eps": {"values": [0.01, 0.05, 0.1]},
        "target_update_interval": {"values": [1_000, 5_000, 10_000]}, # Hard update target net
        "net_arch": {"values": [[64, 64], [128, 128], [256, 128]]},
        # --- Dueling DQN ---
        "dueling_dqn": {"value": True}, 
        # --- Mask Expansion (for the env) ---
        "mask_expansion_strategy": {"value": "none"}, # DQN can't use masks
    },
}

# (You can place this function near your other 'train' functions)

from stable_baselines3 import DQN

def train_dqn(action_ep: str, step_ep: str):
    """Train using DQN with Dueling networks and frame-stacking."""
    
    rand_value = random.randrange(0, 4096)
    run = wandb.init(
        project=os.environ.get("WANDB_PROJECT", "xr-abr-dqn"),
        entity=os.environ.get("WANDB_ENTITY"),
        save_code=True,
        name=f'DQN_T{T_ABR}_obs{observation_type}_{rand_value}'
        # You might want to pull the config from DQN_SWEEP_CONFIG here
        # config=DQN_SWEEP_CONFIG["parameters"] 
    )

    # --- 1. Create the Environment ---
    # We use MaskableDiscreteZmqEnv because it has the discrete action space
    # DQN needs. We do NOT wrap it in ActionMasker, as DQN does not
    # support action masking. The agent will learn to avoid bad actions
    # via the reward signal.
    
    # Get expansion strategy from config (even if DQN doesn't use it, the env needs it)
    expansion_strategy = wandb.config.get("mask_expansion_strategy", "none")
    expansion_param = wandb.config.get("mask_expansion_param", None)

    env = MaskableDiscreteZmqEnv(
        action_ep=action_ep,
        step_ep=step_ep,
        bitrate_ladder_mbps=BITRATE_LADDER_MBPS,
        expansion_strategy=expansion_strategy,
    ...
    print(f"{Colors.GREEN}Environment created with {len(BITRATE_LADDER_MBPS)} discrete actions{Colors.ENDC}")
    print(f"{Colors.YELLOW}NOTE: DQN will ignore action masks from the environment.{Colors.ENDC}")

    # --- 2. Configure Policy ---
    # We enable Dueling DQN for "advanced" Q-learning.
    policy_kwargs = dict(
        net_arch=list(wandb.config.net_arch),
        dueling=wandb.config.get("dueling_dqn", True)
    )
    
    # NOTE: We DO NOT use LastRowExtractor.
    # We want the DQN to see the full, frame-stacked observation
    # (WINDOW_LEN * FEAT_DIM,) to solve the POMDP.
    # The default MlpPolicy will use the full flat vector, which is correct.

    print(f"{Colors.CYAN}DQN Policy: Dueling={policy_kwargs['dueling']}{Colors.ENDC}")
    ...
    model = DQN(
        ...
        train_freq=wandb.config.get("train_freq", 4),
        gradient_steps=wandb.config.get("gradient_steps", 1),
    ...
    print(f"{Colors.GREEN}DQN model created{Colors.ENDC}")
    ...
    # --- 5. Train ---
    print(f"{Colors.YELLOW}{Colors.BLINK}Starting DQN training...{Colors.ENDC}")
    model.learn(total_timesteps=N_STEPS_RL, callback=callback)
    ...
    art = wandb.Artifact(
        name=f"DQN-{run.id}-final",
        type="model",
        description=f"Final DQN after {N_STEPS_RL} steps"
    )
    art.add_file(final_model_path)
    run.log_artifact(art)
    wandb.finish()
