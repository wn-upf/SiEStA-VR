...
class ZmqServer:
    ...
    def __init__(self, action_ep: str, step_ep: str, trainer_ep: str):
        ...
        self.router.setsockopt(zmq.RCVTIMEO, TIMEOUT_ZMQSERVER)  # 1 second timeouts
        self.router.bind(action_ep)
        
        
        self.pull = self.ctx.socket(zmq.PULL)
        self.pull.setsockopt(zmq.RCVTIMEO, TIMEOUT_ZMQSERVER)
        self.pull.bind(step_ep)
        
        # Socket for Python Trainer Client (REP for req/rep)
        self.rep_socket = self.ctx.socket(zmq.REP)
        self.rep_socket.setsockopt(zmq.RCVTIMEO, TIMEOUT_ZMQSERVER)
            ...
            if self.pull in socks:
                ...
                if self.trainer_is_waiting_for_step:
                    ...
                    if self.active_sim_id is not None and transition.get("sim_id") != self.active_sim_id:
                        ...
                        self.active_sim_id = transition.get('sim_id').encode()
                        continue
